...
export function createComponentInstance(
  ...
  const instance: ComponentInternalInstance = {
    ...
    next: null,
    subTree: null!, // 创建之后马上被同步设置
    update: null!, // 创建之后马上被同步设置
    render: null,
    renderProxy: null,
    propsProxy: null,
    setupContext: null,
...
// 打包之前在vue/src/index下调用了此方法，来确定编译的方法
...
function finishComponentSetup(
  ...
  if (__FEATURE_OPTIONS__) {
    currentInstance = instance
    currentSuspense = parentSuspense
    // 处理组件初始化时候传入的参数如data、provide、inject、mixins等
